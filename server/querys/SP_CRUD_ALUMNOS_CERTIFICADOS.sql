...
BEGIN
  ...
  IF ($METODO = 'CREAR_ALUMNO') THEN
    ...
    ELSE
      ...
      SELECT 
        ...
        UPPER(ExtractValue($XMLDOC, '//ALUMNO/NOMBRES')),
        UPPER(ExtractValue($XMLDOC, '//ALUMNO/APELLIDOS')),
  ...
  -- EDITAR CERTIFICADO
  ELSEIF ($METODO = 'EDIT_CERTIFICADO') THEN
  
    /*
      CALL SP_CRUD_ALUMNOS_CERTIFICADOS('CARGAR_CERTIFICADO', '', '', '', '', 
      '<CERTIFICADO>
      <IDCERTIFICADO>34</IDCERTIFICADO>
      <LIBRO>EXCEL</LIBRO>
      <FOLIO>2024</FOLIO>
      <NUMERO>00001</NUMERO>
      <CODALUMNO>000012121212</CODALUMNO>
      <CURSO>EXCEL</CURSO>
      <NOMBRE/>
      <BASE64></BASE64>
      <NOTA>18</NOTA>
    ...
    */
    UPDATE certificados SET 
      LIBRO = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/LIBRO')),
      FOLIO = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/FOLIO')),
      NUMERO = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/NUMERO')),
      CODALUMNO = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/CODALUMNO')),
      CURSO = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/CURSO')),
      NOTA = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/NOTA')),
      FECHA_CERT = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/FECHA_CERT'))
    WHERE IDCERTIFICADO = (SELECT ExtractValue($XMLDOC, '//CERTIFICADO/IDCERTIFICADO'));
